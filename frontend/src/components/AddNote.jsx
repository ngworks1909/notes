  ...
  return (
    <div className='new note display-flex justify-between flex-column'>
        ...
        <div className="note-footer display-flex align-center justify-between">
            ...
            <button className={`save cursor-${(notetitle.trim().length < 4 || noteText.trim().length < 5)? 'disabled':'pointer'}`} disabled={notetitle.length < 4 || noteText.length < 5} onClick ={handleSaveClick}>Save</button>
        </div>
